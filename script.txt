...
function displayTotalCount() {
  ...
  function updateData(index){
    ...
    $('#btnUpdate').click(function(){
        if(nameInput.value.length > 1 && genderInput.value.length > 1 && bdayInput.value.length > 1 && addressInput.value.length > 1){
            residentsList[index].name = nameInput.value.toUpperCase();
            residentsList[index].gender = genderInput.value.toUpperCase();
            residentsList[index].birthday = bdayInput.value;
            residentsList[index].address = addressInput.value.toUpperCase();
            localStorage.setItem('residentsList', JSON.stringify(residentsList));
            addToTable();
            displayTotalCount();
